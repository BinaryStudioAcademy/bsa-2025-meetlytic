...
test.describe("Last Name Validation - Negative Cases", () => {
	test("Digit in LastName field", async () => {
		...
		const digitLastName = generateFakeUser({}, { digitLastName: true }, {}); // Register with digitLastName LastName
	...
	test("Symbol in LastName field", async () => {
		// Generate a valid user
		const symbolLastname = generateFakeUser({}, { symbolLastname: true }, {}); // Register with symbolLastname LastName
	...
	test("Fifty one letters in Lastname field", async () => {
		// Generate a valid user
		const fiftyoneLetterLastName = generateFakeUser(
			{},
			{ fiftyoneLetterLastName: true },
			{},
		); // Register with fiftyoneLetterLastName LastName
	...
	test("Empty Lastname field", async () => {
		// Generate a valid user
		const emptyLastName = generateFakeUser({}, { emptyLastName: true }, {}); // Register with emptyLastName LastName
