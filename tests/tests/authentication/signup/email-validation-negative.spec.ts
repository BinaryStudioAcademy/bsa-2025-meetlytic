...
test.describe('Email Validation - Negative Cases', () => {
	test('Hyphens at the start of domain segments', async () => {
		...
		expect(response.status(), 'Expected HTTP 422 Unprocessable Entity').toBe(HTTP_UNPROCESSABLE_ENTITY); // Check if the response status code is 422 Unprocessable Entity
	});
	...
	test('Duplicate email sign-up returns 409 Conflict', async () => {
		...
		expect(firstResponse.status(), 'Expected HTTP 201 Created').toBe(HTTP_CREATED);

		// Second signup attempt with the same email - expect 409 Conflict
		const secondResponse = await api.auth.sign_up(validUser);
		expect(secondResponse.status(), 'Expected HTTP 409 Conflict').toBe(HTTP_CONFLICT);
	});
});
